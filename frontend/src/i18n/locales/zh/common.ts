export default {
  ...
  landingHome: {
    ...
    login: '登录',
    console: '进入控制台 ↗',
    heroTitleLine1: '连接 AI，',
    heroTitleLine2: '让灵感自由生长。',
    heroIntroLine1: '从第一行代码，到下一幅作品。',
    heroIntroLine2: '用一个 API 入口，把模型能力融入你的工作流。',
    heroIntroLine3: '少一点配置，多一点创造。',
    start: '开始接入',
    viewExample: '查看接入示例',
    heroNote: 'API 接入 / 编程助手 / 无限画布',
    conceptLabel: '统一 API 入口连接代码、对话、图像和创作工作流的概念示意图',
    modelsHint: '探索多模型生态',
    modelsDisclaimer: '具体可用模型以控制台为准',
    possibilitiesTitle: '一个入口，不止一种可能。',
    possibilitiesDescription: '写代码、做产品，或把脑海里的画面变成作品。',
    possibilitiesPrompt: '找到适合你的开始方式。',
    developTitle: '让工具，接上能力。',
    developDescription: '在兼容的 SDK 与开发工具中配置接口地址和 API Key，延续你熟悉的开发习惯。',
    viewIntegration: '查看接入方式 ↗',
    exploreTitle: '把选择权，交给你。',
    exploreDescription: '按任务选择模型，在控制台管理密钥与查看用量。具体模型、计费及权限以站内信息为准。',
    exploreConsole: '探索控制台 ↗',
    createTitle: '灵感，不必排成一行。',
    createDescription: '进入无限画布，把图像与想法放在一起，让创作有更大的展开空间。',
    openCanvas: '打开无限画布 ↗',
    requestTitle: '从一个请求，开始。',
    requestDescription: '不用重新学习一套工作方式。',
    requestPrompt: '沿用熟悉的 OpenAI SDK，配置你的接入信息。',
    stepCreateKey: '登录控制台，创建 API Key',
    stepSelectModel: '选择分组可用的模型',
    stepSendRequest: '替换配置，发起第一个请求',
    exampleLanguage: '示例语言',
    copyCode: '复制代码',
    codeDisclaimer: '接入示例 · 请替换 API Key 与模型名称 · 此页面不会发送请求',
    faqTitle: '开始之前。',
    faqToolsQuestion: '我需要更换现有的开发工具吗？',
    faqToolsAnswer: '通常不需要。支持自定义 Base URL 的兼容客户端可以使用本站入口。不同工具和模型支持的接口可能不同，接入前请确认控制台说明。',
    faqPricingQuestion: '模型和费用在哪里查看？',
    faqPricingAnswer: '请在实际控制台查看分组、可用模型和计费信息。可用模型与费用以当前分组实际信息为准。',
    faqCanvasQuestion: '无限画布如何使用？',
    faqCanvasAnswer: '登录后打开创作空间，选择有相应模型权限的 API Key。图片等请求会按所选分组的实际规则计费。',
    finalTitle: '下一件好作品，从这里开始。',
    finalDescription: '把复杂留在接口之后，把专注留给你的想法。',
    createKey: '创建你的 API Key ↗',
    copied: '代码已复制，请替换密钥与模型名称',
    selected: '代码已选中，请按 Ctrl/Cmd + C 复制',
  },
