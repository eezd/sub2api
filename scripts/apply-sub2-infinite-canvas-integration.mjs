...
export async function reconcileInfiniteCanvasLocaleBlock(file, block, check = false) {
  ...
  const legacyCanvasBlock = currentBlock.includes("notConfigured:") && !currentBlock.includes("frameTitle:")
  if (check || !legacyCanvasBlock) throw new Error(`Infinite Canvas locale block drift in ${file}`)
...
export async function applySub2InfiniteCanvasIntegration({ root, check = false }) {
  ...
  const routerFile = path.join(resolvedRoot, 'frontend/src/router/index.ts')
  const routerContent = await readFile(routerFile, 'utf8')
  if (!routerContent.includes("path: '/workspace/canvas'")) {
    await ensureReplace(
      routerFile,
      "  {\n    path: '/batch-image',\n",
      `  {
...
`,
  ...
  for (const locale of localePatches) {
    ...
    const localeContent = await readFile(file, 'utf8')
    if (!localeContent.includes(navSentinel)) {
  ...
  const securityHeadersContent = await readFile(securityHeadersFile, 'utf8')
  if (!securityHeadersContent.includes('{"frame-src", "\'self\'"}') || !securityHeadersContent.includes('{"connect-src", "http://127.0.0.1:17371"}')) {
    await ensureReplace(
      securityHeadersFile,
