...
export async function patchAntdModalStyles(file) {
  let content
  try {
    content = await readFile(file, 'utf8')
  } catch (error) {
  ...
  await writeFile(file, content.replace(marker, ''), 'utf8')
  return true
}
...
export async function applyInfiniteCanvasPatches({ root }) {
  ...
  await patchAntdModalStyles(path.join(resolvedRoot, 'web/src/components/layout/model-script-editor.tsx'))
